 ...
 * 
 * !!! THIS HAS ONLY BEEN TESTED ON RECTANGULAR ROOMS. DO NOT ATTEMPT TO CREATE NON-RECTANGULAR ROOMS !!!
 */
public class Room extends GridPane {
    
    // A lookup table to translate between letters in .room files and sprites in game
    ...
    // A cache for all rooms that have been loaded before so that we don't have to read from disk every time.
    // I have not tested this at all and I don't even know if the performance is worth the memory or if this even
    // takes up a lot of memory, but its an idea anyway. Actually, this thing isn't even implemented soooo
    ...
    ArrayList<Collidable> bodies = new ArrayList<Collidable>();
    /*
     * Room will also hold data on the locations of items and enemies inside of it for Level to place into layers
     * Level will only keep one room loaded at a time, so this will allow for object permanance
     */
    ...
    public Room(String template) {
        super();
        setAlignment(Pos.CENTER); // By default GridPanes are aligned to the top-left, which we don't want
        ...
        for (int row = 0; row < lines.size(); ++row) {
            for (int col = 0; col < lines.get(row).length(); ++col) {
                ...
                if (img == null) {
                    System.err.println("Detected an invalid character in room " + template + "! Please check file for errors!");
                }
        ...
        if (cache != null) {
            roomCache.put(template, (GridPane) this); // This would probably be better if we cloned it instead of copying a reference...
        }
    }
